 ...
 * @remark モデル中のレイヤーのパラメーターなどが不正でした。ailiaGetErrorDetail()
 * で詳細なエラーメッセージを確認し、モデルファイルが正しいかを確認してください。
 *
 * \~english
 * @def AILIA_STATUS_INVALID_LAYER
 * @brief Incorrect layer weight, parameter, or input or output shape
 * @remark The layer of model had incorrect parameter or so on. Please call ailiaGetErrorDetail() and check detail
 * message. And, please check model.
 */
 ...
 * @remark The specified element of passed name/index was not found. Please check the element are exisit on model or
 * not.
 */
 ...
 * @remark The called API are not allowed on this model (e.g. encrypted model are used.). Please check model file and
 * change API call flow.
 */
 ...
 * @remark The shape (e.g. output shape) are not determined. When called API that to get output shape, please set input
 * shape and execute inference, then call API that to get output shape.
 */
 ...
 * @remark The specified information was removed due to optimization. If you need the information, please disable
 * optimization and call API.
 */
 ...
 * @remark The license file that are required for trial version are broken. Please contact support desk that described
 * on document.
 */
 ...
 * @remark The license file that are required for trial version are expired. Please contact support desk that described
 * on document.
 *
 */
 ...
 */
int AILIA_API
ailiaPredict(struct AILIANetwork* net, void* dest, unsigned int dest_size, const void* src, unsigned int src_size);
...
/**
 * \~japanese
 * 中間バッファーを共有して推論する。 \ref AILIA_MEMORY_REDUCE_INTERSTAGE
 * と併用した場合、共有可能な中間バッファーは開放しない。
 *
 * \~english
 * Infer by sharing the intermediate buffer. When used with  \ref AILIA_MEMORY_REDUCE_INTERSTAGE , the sharable
 * intermediate buffer is not opened.
 */
