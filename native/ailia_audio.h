 ...
 * @brief STFT の際、sample_n の前後に fft_n/2 の padding(zero)
 * を入れ、さらにhop_n処理単位になるように後方padding(zero)で合わせる
 *
 * \~english
 * @def AILIA_AUDIO_STFT_CENTER_SCIPY_DEFAULT
 * @brief for the STFT, insert a padding (zeros) of fft_n/2 before and after the sample_n samples, and also pad at the
 * end with zeros to process in units of hop_n
 */
 ...
 * \~english
 * @def AILIA_AUDIO_FILTFILT_PAD_ODD
 * @brief During zero-phase filtering, pad with an odd reflection (substract the reflected values from two times the
 * edge value)
 */
 ...
 */
int AILIA_API
ailiaAudioConvertToMel(void* dst, const void* src, const void* fb_mtrx, int freq_n, int frame_n, int mel_n);
 ...
 */
int AILIA_API
ailiaAudioFixFrameLen(void* dst, const void* src, int freq_n, int dst_frame_n, int src_frame_n, float pad_data);
 ...
 */
int AILIA_API
ailiaAudioResample(void* dst, const void* src, int dst_sample_rate, int dst_n, int src_sample_rate, int src_n);
